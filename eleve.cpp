...
QSqlQuery query;
        ...
        query.prepare("UPDATE eleve SET ID=:ID ,NOM=:NOM,PRENOM=:PRENOM,DATEN=:DATEN,SEXE=:SEXE,CLASSE=:CLASSE where ID=:ID");
        query.bindValue(":ID", ID);
        query.bindValue(":NOM",NOM);
         query.bindValue(":PRENOM",PRENOM);
         query.bindValue(":DATEN",DATEN);
...
QSqlQueryModel *Eleve::trier(QString x)
{
    ...
    if(x=="nom (A->Z)")
        ...
        model->setQuery("select*  from ELEVE order by ID desc");
    else if (x=="Default")
            model->setQuery("select * from ELEVE");
